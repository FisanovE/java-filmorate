...
public class FilmDbStorage implements FilmStorage {
    ...
    public Film updateFilm(Film film) {
        try {
            String sqlRequest = "UPDATE films SET name = ?, description = ?, release_date = ?, duration = ? " +
                    "WHERE film_id = ?";
            jdbcTemplate.update(sqlRequest, film.getName(), film.getDescription(), film.getReleaseDate(),
    ...
    private Mpa getMpaFromDataBase(Long id) {
        Mpa mpa = null;
        String sqlMpa = "SELECT mpa_id, mpa_name FROM mpa WHERE mpa_id IN (SELECT mpa_id FROM films_mpa " +
                "WHERE film_id = ?)";
    ...
    private List<Genre> getGenresFromDataBase(Long id) {
        String sql = "SELECT genre_id, genre_name FROM genres WHERE genre_id IN (SELECT genre_id " +
                "FROM films_genres WHERE film_id = ?)";
        List<Genre> list = jdbcTemplate.query(sql, (rs, rowNum) -> Genre.builder()
    ...
    public Collection<Film> searchFilms(String query, String by) {
        ...
        if (Objects.equals(by, "director")) {
            ...
            sql = "SELECT f.film_id, f.name, f.description, f.release_date, f.duration, COUNT(l.film_id) AS likes_count " +
                    ...
                    "fd.director_id = d.director_id LEFT JOIN likes l ON f.film_id = l.film_id " +
                    "WHERE LOWER (d.director_name) " +
                    "LIKE LOWER ('%" + query + "%') OR LOWER (f.name) LIKE LOWER ('%" + query + "%') " +
                    "GROUP BY f.film_id, f.name, " + "f.description, f.release_date, f.duration " +
    ...
    private void checkContainsFriendsOfUser(Long userId, Long friendId) {
        SqlRowSet sqlRows = jdbcTemplate
                .queryForRowSet("SELECT * FROM friends WHERE user_id = ? AND friend_id = ?", userId, friendId);
        if (sqlRows.first()) {
