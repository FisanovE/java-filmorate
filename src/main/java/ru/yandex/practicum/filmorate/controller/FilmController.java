...
public class FilmController {
    ...
    private void checkingFilmForValid(Film film) throws ValidationException {
        ...
        if (film.getDescription().length() > 200) {
            throw new ValidationException("The maximum description length is 200 characters, you have: \"" +
                    film.getDescription().length() + "\" characters");
        }
        if (film.getReleaseDate().isBefore(LocalDate.parse("1895-12-28", DateUtils.formatter))) {
            throw new ValidationException("Movie release date should not be earlier than 1895.12.28, you have: \""
                    + film.getReleaseDate() + "\"");
        }
        if (film.getDuration() < 0) {
            throw new ValidationException("The duration of the film should be positive, you have:  \""
                    + film.getDuration());
        }
    }
    ...
    public Collection<Film> searchFilms(@RequestParam String query, @RequestParam String by) {
        ...
        if (Objects.equals(by, "director") || Objects.equals(by, "title") ||
                Objects.equals(by, "title,director") || Objects.equals(by, "director,title")) {
