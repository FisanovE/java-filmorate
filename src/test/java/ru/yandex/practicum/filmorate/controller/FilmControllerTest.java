...
class FilmControllerTest {
    ...
    void shouldReturnExceptionWhenNameNewFilmIsEmpty() {
        ...
        assertAll(() -> assertEquals("Invalid title format: \"" + notValidName + "\"", exception.getMessage()), () -> assertTrue(list.isEmpty()));
    }
    ...
    void shouldReturnExceptionWhenDescriptionNewFilmMoreThan200Char() {
        Film filmNew = createFilm();
        String notValidDescription = "This is a pretty famous film that no one has ever seen. That's exactly what he's " + "famous for. Critics do not respond to him in any way, because they have not seen him once. " + "The audience is enthusiastically silent about him for the same reason.";
        filmNew.setDescription(notValidDescription);
        List<Film> list = new ArrayList<>(controller.getAllFilms());
        final ValidationException exception = assertThrows(ValidationException.class, () -> controller.addNewFilm(filmNew));
        assertAll(() -> assertEquals("The maximum description length is 200 characters, you have: \"" + notValidDescription.length() + "\" characters", exception.getMessage()), () -> assertTrue(list.isEmpty()));
    }
    ...
    void shouldReturnExceptionWhenReleaseDateNewFilmIsNotValid() {
        ...
        List<Film> list = new ArrayList<>(controller.getAllFilms());
        final ValidationException exception = assertThrows(ValidationException.class, () -> controller.addNewFilm(filmNew));
        assertAll(() -> assertEquals("Movie release date should not be earlier than 1895.12.28, you have: \"" + notValidReleaseDate + "\"", exception.getMessage()), () -> assertTrue(list.isEmpty()));
    }
    ...
    void shouldReturnExceptionWhenDurationNewFilmIsNotValid() {
        ...
        List<Film> list = new ArrayList<>(controller.getAllFilms());
        final ValidationException exception = assertThrows(ValidationException.class, () -> controller.addNewFilm(filmNew));
        assertAll(() -> assertEquals("The duration of the film should be positive, you have:  \"" + notValidDuration, exception.getMessage()), () -> assertTrue(list.isEmpty()));
    }
    ...
    void shouldUpdateFilm() {
        ...
        assertAll(() -> assertFalse(list.isEmpty(), "Film not added"), () -> assertEquals(filmAdded, list.get(0), "Films are not equal"));
    }
    ...
    void shouldAddLike() {
        ...
        assertAll(() -> assertFalse(likes.isEmpty(), "Like not added"), () -> assertEquals(2, likes.get(0), "Films id are not equal"));
    }
    ...
    void shouldReturnTopRatingFilms() {
        ...
        assertAll(() -> assertFalse(films.isEmpty(), "Rating list must by not empty"), () -> assertEquals(filmAdded2.getId(), films.get(0)
                .getId(), "Films id are not equal"), () -> assertEquals(2, films.size(), "List`s size not equal 2"));
    }
