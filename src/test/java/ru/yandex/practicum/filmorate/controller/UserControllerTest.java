...
class UserControllerTest {
	...
	void shouldReturnExceptionWhenEmailNewUserIsNotValid() {
		...
		List<User> list = new ArrayList<>(controller.getAllUsers());
		final ValidationException exception = assertThrows(ValidationException.class, () -> controller.addNewUser(userNew));
	...
	void shouldReturnExceptionWhenLoginNewUserIsNotValid2() {
		...
		List<User> list = new ArrayList<>(controller.getAllUsers());
		final ValidationException exception = assertThrows(ValidationException.class, () -> controller.addNewUser(userNew));
		assertAll(() -> assertEquals("Login field must not be empty and contain spaces: \""
				+ notValidLogin + "\"", exception.getMessage()),
				() -> assertTrue(list.isEmpty()));
	}
	...
	void shouldReturnExceptionWhenBirthdayNewUserIsNotValid() {
		...
		List<User> list = new ArrayList<>(controller.getAllUsers());
		final ValidationException exception = assertThrows(ValidationException.class, () -> controller.addNewUser(userNew));
		assertAll(() -> assertEquals("Date of birth cannot be in the future: \""
				+ notValidBirthday + "\"", exception.getMessage()),
				() -> assertTrue(list.isEmpty()));
	}
	...
	void shouldAddFriend() throws ValidationException {
		...
		assertAll(() -> assertFalse(list1.isEmpty(), "Friend of User1 is not added"),
	...
	void shouldDeleteFriend() throws ValidationException {
		...
		assertAll(() -> assertTrue(list1.isEmpty(), "User 1's friend list must be empty"),
	...
	void shouldAllFriendsByUser() throws ValidationException {
		...
		assertAll(() -> assertFalse(list.isEmpty(), "User's friend list must be not empty"),
